...
size_t DefaultKeyboardDE::set(KeyboardKeycodeDE k, bool s)
{
	// It's a modifier key
	if(k >= DE_KEY_LEFT_CTRL && k <= DE_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeDE(uint8_t(k) - uint8_t(DE_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = DE_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardDE::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardDE::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardDE::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardDE::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardDE::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardDE::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
