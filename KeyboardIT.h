...
public:
  ...
  inline size_t write(KeyboardKeycodeIT k);
  inline size_t press(KeyboardKeycodeIT k);
  inline size_t release(KeyboardKeycodeIT k);
  inline size_t remove(KeyboardKeycodeIT k);
  inline size_t add(KeyboardKeycodeIT k);
...
private:
  virtual size_t set(KeyboardKeycodeIT k, bool s) = 0;
  inline size_t set(uint8_t k, bool s);
};


// Implementation is inline
#include "KeyboardIT.hpp"
