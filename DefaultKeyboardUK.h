...
typedef union ATTRIBUTE_PACKED {
	...
	struct ATTRIBUTE_PACKED {
		uint8_t modifiers;
		uint8_t reserved;
		KeyboardKeycodeUK keycodes[6];
	};
	uint8_t keys[8];
} HID_KeyboardReport_Data_tUK;

class DefaultKeyboardUK : public KeyboardUK
{
public:
  ...
  using KeyboardUK::write;
  using KeyboardUK::press;
  using KeyboardUK::release;
  using KeyboardUK::add;
  using KeyboardUK::remove;
...
protected:
  HID_KeyboardReport_Data_tUK _keyReport;

private:
  inline virtual size_t set(KeyboardKeycodeUK k, bool s) override;
};

// Implementation is inline
#include "DefaultKeyboardUK.hpp"
