...
size_t DefaultKeyboardTR::set(KeyboardKeycodeTR k, bool s)
{
	// It's a modifier key
	if(k >= TR_KEY_LEFT_CTRL && k <= TR_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeTR(uint8_t(k) - uint8_t(TR_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = TR_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardTR::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardTR::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardTR::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardTR::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardTR::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardTR::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
