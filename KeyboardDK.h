...
public:
  ...
  inline size_t write(KeyboardKeycodeDK k);
  inline size_t press(KeyboardKeycodeDK k);
  inline size_t release(KeyboardKeycodeDK k);
  inline size_t remove(KeyboardKeycodeDK k);
  inline size_t add(KeyboardKeycodeDK k);
...
private:
  virtual size_t set(KeyboardKeycodeDK k, bool s) = 0;
  inline size_t set(uint8_t k, bool s);
};


// Implementation is inline
#include "KeyboardDK.hpp"
