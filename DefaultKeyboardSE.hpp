...
size_t DefaultKeyboardSE::set(KeyboardKeycodeSE k, bool s)
{
	// It's a modifier key
	if(k >= SE_KEY_LEFT_CTRL && k <= SE_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeSE(uint8_t(k) - uint8_t(SE_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = SE_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardSE::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardSE::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardSE::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardSE::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardSE::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardSE::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
