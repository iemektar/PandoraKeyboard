...
size_t DefaultKeyboardUK::set(KeyboardKeycodeUK k, bool s)
{
	// It's a modifier key
	if(k >= UK_KEY_LEFT_CTRL && k <= UK_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeUK(uint8_t(k) - uint8_t(UK_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = UK_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardUK::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardUK::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardUK::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardUK::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardUK::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardUK::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
