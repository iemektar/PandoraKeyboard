...
typedef union ATTRIBUTE_PACKED {
	...
	struct ATTRIBUTE_PACKED {
		uint8_t modifiers;
		uint8_t reserved;
		KeyboardKeycodeTR keycodes[6];
	};
	uint8_t keys[8];
} HID_KeyboardReport_Data_tTR;

class DefaultKeyboardTR : public KeyboardTR
{
public:
  ...
  using KeyboardTR::write;
  using KeyboardTR::press;
  using KeyboardTR::release;
  using KeyboardTR::add;
  using KeyboardTR::remove;
...
protected:
  HID_KeyboardReport_Data_tTR _keyReport;

private:
  inline virtual size_t set(KeyboardKeycodeTR k, bool s) override;
};

// Implementation is inline
#include "DefaultKeyboardTR.hpp"
