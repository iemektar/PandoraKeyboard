...
public:
  ...
  inline size_t write(KeyboardKeycodeDE k);
  inline size_t press(KeyboardKeycodeDE k);
  inline size_t release(KeyboardKeycodeDE k);
  inline size_t remove(KeyboardKeycodeDE k);
  inline size_t add(KeyboardKeycodeDE k);
...
private:
  virtual size_t set(KeyboardKeycodeDE k, bool s) = 0;
  inline size_t set(uint8_t k, bool s);
};


// Implementation is inline
#include "KeyboardDE.hpp"
