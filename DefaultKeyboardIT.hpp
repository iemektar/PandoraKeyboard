...
size_t DefaultKeyboardIT::set(KeyboardKeycodeIT k, bool s)
{
	// It's a modifier key
	if(k >= IT_KEY_LEFT_CTRL && k <= IT_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeIT(uint8_t(k) - uint8_t(IT_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = IT_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardIT::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardIT::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardIT::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardIT::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardIT::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardIT::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
