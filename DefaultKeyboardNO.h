...
typedef union ATTRIBUTE_PACKED {
	...
	struct ATTRIBUTE_PACKED {
		uint8_t modifiers;
		uint8_t reserved;
		KeyboardKeycodeNO keycodes[6];
	};
	uint8_t keys[8];
} HID_KeyboardReport_Data_tNO;

class DefaultKeyboardNO : public KeyboardNO
{
public:
  ...
  using KeyboardNO::write;
  using KeyboardNO::press;
  using KeyboardNO::release;
  using KeyboardNO::add;
  using KeyboardNO::remove;
...
protected:
  HID_KeyboardReport_Data_tNO _keyReport;

private:
  inline virtual size_t set(KeyboardKeycodeNO k, bool s) override;
};

// Implementation is inline
#include "DefaultKeyboardNO.hpp"
