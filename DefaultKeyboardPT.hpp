...
size_t DefaultKeyboardPT::set(KeyboardKeycodePT k, bool s)
{
	// It's a modifier key
	if(k >= PT_KEY_LEFT_CTRL && k <= PT_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodePT(uint8_t(k) - uint8_t(PT_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = PT_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardPT::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardPT::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardPT::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardPT::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardPT::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardPT::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
