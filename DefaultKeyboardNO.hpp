...
size_t DefaultKeyboardNO::set(KeyboardKeycodeNO k, bool s)
{
	// It's a modifier key
	if(k >= NO_KEY_LEFT_CTRL && k <= NO_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeNO(uint8_t(k) - uint8_t(NO_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = NO_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardNO::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardNO::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardNO::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardNO::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardNO::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardNO::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
