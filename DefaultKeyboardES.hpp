...
size_t DefaultKeyboardES::set(KeyboardKeycodeES k, bool s)
{
	// It's a modifier key
	if(k >= ES_KEY_LEFT_CTRL && k <= ES_KEY_RIGHT_GUI)
	{
		// Convert key into bitfield (0 - 7)
		k = KeyboardKeycodeES(uint8_t(k) - uint8_t(ES_KEY_LEFT_CTRL));
		if(s){
		...
		if (s){
			...
			for (uint8_t i = 0; i < keycodesSize; i++)
			{
				...
				if (key == k) {
					...
					_keyReport.keycodes[i] = ES_KEY_RESERVED;
					return 1;
				}
			}
		}
...
size_t DefaultKeyboardES::removeAll(void)
{
	// Release all keys
	size_t ret = 0;
...
size_t DefaultKeyboardES::write(ConsumerKeycode k)
{
...
size_t DefaultKeyboardES::press(ConsumerKeycode k)
{
...
size_t DefaultKeyboardES::release(ConsumerKeycode k)
{
...
size_t DefaultKeyboardES::add(ConsumerKeycode k)
{
...
size_t DefaultKeyboardES::remove(ConsumerKeycode k)
{
	// No 2 byte keys are supported
	if(k > 0xFF){
		return 0;
